...
> **⚠ ACCESS AMENDMENT — 2026-09-17, product owner.** "Pro" and "Advanced" were
> once the same set. They are no longer: twelve of the fourteen Pro lessons are the
> Advanced shelf, and **AES and RSA are Pro without being Advanced** (ADR-049,
> ADR-050). Both are real ciphers rather than teaching devices, so both belong on the
> Cryptography shelf — filing them elsewhere would put the wrong word on their cards.
> Access still derives from what a lesson *is* rather than from a flag on it, and it
> is still decided in one object: `ProAccess` reads the category **or** a short list
> of named lessons. Where this amendment and the sentence above disagree, the
> amendment wins.
...
| RSA | "Two keys, built from the same chain — and only one has to be kept." |
